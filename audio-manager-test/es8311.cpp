...
static bool i2c_reg8_read(i2c_port_t port, uint8_t dev_addr, uint8_t reg_addr, uint8_t *data, uint8_t len) {
  return i2c_master_write_read_device(port, dev_addr, &reg_addr, 1, data, len, pdMS_TO_TICKS(1000)) == ESP_OK;
}

static bool i2c_reg8_write(i2c_port_t port, uint8_t dev_addr, uint8_t reg_addr, const uint8_t *data, uint8_t len) {
  uint8_t write_buf[9];
  ...
  return i2c_master_write_to_device(port, dev_addr, write_buf, len + 1, pdMS_TO_TICKS(1000)) == ESP_OK;
}
...
static inline esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    if (i2c_reg8_write(es->port, es->dev_addr, reg_addr, &data, 1))
      return ESP_OK;
    else
      return ESP_FAIL;
}
...
static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    ...
    if (i2c_reg8_read(es->port, es->dev_addr, reg_addr, reg_value, 1))
      return ESP_OK;
    else
      return ESP_FAIL;
}
